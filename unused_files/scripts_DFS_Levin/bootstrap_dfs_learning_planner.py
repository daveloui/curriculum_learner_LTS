...
class BootstrapDFSLearningPlanner ():
    ...
    def learn(self):
        ...
        random_indices = np.random.permutation (
            len (self._y))  # FD: create some random permutation of integers 0 to len(self._y)
        ...
        for i in range (len (self._y)):
            ...
            if index_beg < len (random_indices):
                ...
                error = self.conv_nn.batch_train_positive_examples (x,
                                                                    y)  # This is where we get the error. Is the policy trained here?
                errors.append (error)
    ...
    def _dfs_lvn_budget_for_learning(self, state, p, depth, level):
        ...
        for a in actions:
            ...
            if child.is_solution ():
                self.solution_probability = math.exp (
                    p)  # Adding the probability of finding this solution to difficulty ordering log
