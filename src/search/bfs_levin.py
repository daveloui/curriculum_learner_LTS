...
class BFSLevin ():
    ...
    def get_levin_cost(self, child_node, predicted_h):
        if self._use_learned_heuristic and self._use_heuristic:
            ...
            return math.log (
                child_node.get_game_state ().heuristic_value () + child_node.get_g ()) - child_node.get_p ()
    ...
    def search(self, data):
        ...
        while len (_open) > 0:
            ...
            for a in actions:
                ...
                child_node = TreeNode (node, child, node.get_p () + probability_distribution[a], node.get_g () + 1, -1,
                                       a)
