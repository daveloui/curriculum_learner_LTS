...
class PuzzleGenerator:
    ...
    def generate_puzzles_with_random_paths(self, puzzle_size, bullet_probability, color_bullets, time_limit,
                                           puzzle_folder, number_puzzles, ncpus):
        """
        ...
        while time.time () - start_time < time_limit - 10 and len (puzzles_generated) < number_puzzles:
            ...
            with ProcessPoolExecutor (max_workers=ncpus) as executor:
                args = ((puzzle_size, minimum_number_regions, color_bullets, bullet_probability) for _ in
                        range (10 * ncpus))
        ...
        for puzzle in puzzles_generated:
            puzzle.save_state (
                puzzle_folder + '/' + str (puzzle_size[0]) + 'x' + str (puzzle_size[1]) + '_' + str (puzzle_id))
