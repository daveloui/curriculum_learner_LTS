    ...
    def _solve_uniform_online(self, planner, nn_model, parameters):
        ...
        while len (current_solved_puzzles) < self._number_problems:
            ...
            if n_P > 0:
                # if self.use_GPUs:
                # with tf.device ('/GPU:0'):
                retrieve_batch_data_solved_puzzles (P,
                                                    memory_v2)  # TODO: used to return: batch_images_P, batch_actions_P
                # retrieve_batch_data_solved_puzzles also stores images_P and actions_P in dictionary (for each puzzle)

                theta_diff = compute_cosines (nn_model, self._models_folder, while_loop_iter)  # used to return: cosine, dot_prod, theta_diff
                # levin_cost, average_levin_cost, training_loss = compute_levin_cost (batch_images_P, batch_actions_P, nn_model)
                argmax_p_dot_prods, Rank_dot_prods, argmax_p_cosines, Rank_cosines = compute_rank (P, nn_model,
                                                                                                   theta_diff,
                                                                                                   memory_v2,
                                                                                                   self._ncpus, 19, n_P,
                                                                                                   parallelize_with_NN)
                argmin_p_levin_score, Rank_levin_scores = compute_rank_mins (P, nn_model, memory_v2, self._ncpus, 19,
                                                                             n_P,
                                                                             parallelize_with_NN)  # How is the ordering different if we use argmin? How is the ranking different?
        ...
        if len (current_solved_puzzles) == self._number_problems:  # and iteration % 2 != 0.0:
            save_data_to_disk (Rank_max_dot_prods,
                               join (self._ordering_folder, 'Rank_MaxDotProd_BFS_' + str (self._puzzle_dims) + ".pkl"))
            save_data_to_disk (Rank_max_cosines,
                               join (self._ordering_folder, 'Rank_MaxCosines_BFS_' + str (self._puzzle_dims) + ".pkl"))
            ...
            save_data_to_disk (indexes_rank_data,
                               join (self._ordering_folder, 'Idxs_rank_data_BFS_' + str (self._puzzle_dims) + ".pkl"))
            ...
            save_data_to_disk (ordering_cosines,
                               join (self._ordering_folder, 'Ordering_Cosines_BFS_' + str (self._puzzle_dims) + ".pkl"))
            save_data_to_disk (ordering_levin_scores, join (self._ordering_folder, 'Ordering_LevinScores_BFS_' + str (
                self._puzzle_dims) + ".pkl"))
